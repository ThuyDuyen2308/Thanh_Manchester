...
namespace PhanMemQuanLyQuanCaffe
{
    public partial class frmNhanVien : Form
    {
        ...
        private void menuThem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Chỉ Admin mới được thêm nhân viên!", "Từ Chối",
            ...
            if (mk < 0)
            {
                MessageBox.Show("Mật khẩu không được âm!", "Lỗi",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtMatKhau.Focus();
                return;
            }

            // Kiểm tra SDT nếu có nhập
            if (!string.IsNullOrWhiteSpace(txtSDT.Text) &&
                !System.Text.RegularExpressions.Regex.IsMatch(txtSDT.Text.Trim(), @"^0\d{9}$"))
            {
                MessageBox.Show("Số điện thoại không hợp lệ!\n(Phải 10 số, bắt đầu bằng 0)", "Lỗi",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtSDT.Focus();
                return;
            }
            ...
            int tonTai = Convert.ToInt32(ConnectSQL.ExecuteScalar(
            ...
            int kq = ConnectSQL.RunQuery(
                "INSERT INTO NhanVien (MaNV,TenNV,MatKhau,SDT,DiaChi,Quyen) VALUES (@ma,@ten,@mk,@sdt,@dc,@quyen)",
                new SqlParameter("@ma", txtMaNV.Text.Trim()),
